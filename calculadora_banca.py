...
if st.button("Calcular Agenda"):
    if banca_inicial > 0 and meta_desejada > 0 and dias_para_meta > 0:
        ...
        ax.set_facecolor('#0d1216')  # Fundo preto do gráfico
        ...
        fig.savefig(grafico_buffer, format="png")  # Removendo o transparent=True para manter o fundo preto
...
with col1:
    st.markdown(
        ...
        'border-radius: 5px; padding: 10px 15px; font-size: 16px; text-decoration: none;">'
        'Criar Conta Polarium</a>', unsafe_allow_html=True)

with col2:
    st.markdown(
        '<a href="https://www.tradingview.com/?aff_id=47934" target="_blank" '
        'style="background-color: #0d1216; color: #ffffff; font-weight: bold; border: 2px solid #ff4b4b; '
        'border-radius: 5px; padding: 10px 15px; font-size: 16px; text-decoration: none;">'
        'Criar Conta TradingView</a>', unsafe_allow_html=True)
